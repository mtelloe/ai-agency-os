...
export async function POST(request: NextRequest) {
  try {
    ...
    const spent = await spendCredit(workspaceId, userId, 'propuesta', `Propuesta para ${auditoria.url}`, token);
    ...
    const { data: propuesta, error } = await db
      .from('propuestas')
      .insert({
    ...
    await logActivity(workspaceId, userId, 'propuesta_generada', `Propuesta generada: ${proposal.titulo}`, token, 'propuestas', propuesta.id);

    return NextResponse.json(propuesta);
  } catch (error) {
    console.error('Generate proposal error:', error);
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Error al generar propuesta' }, { status: 500 });
  }
}
